  ...
  <div class="muro">
    <div class="profile-content">
      ...
      <div class="publications">
        <div v-for="(publication, index) in displayedPublications" :key="index" class="publication">
          <div class="publication-user">
            <img :src="displayedPublications.avatar" alt="profile" />
            <div class="publication-user-name">
              <h3>@{{ displayedPublications.text }}</h3>
        ...
        <button v-if="hasMorePublications" @click="loadMore">Cargar más</button>
      </div>
    </div>
  </div>
</template>
...
import useNext from "@/composables/useNext"

const { handleGetDataProfilePublic } = useProfilePublic()
const { handleGetPublications } = useNext()
...
export default {
    ...
    computed: {
        hasMorePublications() {
            return this.displayedPublications.length < this.publications.length;
        },
    },
    methods: {
        ...
        async fetchPublications() {
            const publications = await handleGetPublications()
            this.publications = publications.items
            this.totalItems = publications.total_items
            this.loadMore()
        },
        handleScroll(event) {
        ...
        loadMore() {
            const morePublications = this.publications.slice(this.displayedPublications.length, this.displayedPublications.length + this.limit);
            this.displayedPublications.push(...morePublications);
        },
